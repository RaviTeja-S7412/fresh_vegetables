...
public function consumersOrders(){
	...
	$fsorders = $this->db->query("select order_id,user_id,product_id,qty,shipping_address,delivery_status,id,order_type,order_status,invoice_number,assigned_to,delivery_date as deliverydate,order_date as date_of_order,deliveryShift from tbl_free_sample_orders order by id desc")->result();
	   ...
	   if($u->order_type == "subscribe"){
		 ...
		 foreach($sdata as $sd){
		   ...
		   foreach($orderproducts as $op1){
			  ...
			  $gst1 = $op1->gst/2; 
			  $nAmt1 = $op1->price * $op1->qty; 
			  $gstAmount1 = ($nAmt1 * $op1->gst) / 100;
		   ...
		   if($u->order_status == "Success"){
			   ...
			   foreach($orderproducts as $op){
				  ...
				  $gst = $op->gst/2; 
				  $nAmt = $op->price * $op->qty; 
				  $gstAmount = ($nAmt * $op->gst) / 100;
...
//	$orders = $this->db->query("select order_id,user_id,shipping_address,delivery_status,order_type,date_of_order,assigned_to,id,invoice_number,deliveryonce_date as deliverydate from orders where payment_status='Success' and order_status='Success' order by id desc")->result();
	...
	$this->db->select('order_id,user_id,shipping_address,delivery_status,order_type,order_status,date_of_order,sub_start_date,invoice_number,deliveryShift,assigned_to,deliveryonce_date as deliverydate');
	$this->db->from('orders');
...
//	$fsorders = $this->db->query("select order_id,user_id,product_id,qty,shipping_address,delivery_status,id,order_type,assigned_to,delivery_date as deliverydate,order_date as date_of_order,deliveryShift from tbl_free_sample_orders where order_status='Success' order by id desc")->result();
	
	$this->db->select('order_id,user_id,product_id,qty,shipping_address,order_status,invoice_number,delivery_status,id,order_type,assigned_to,delivery_date as deliverydate,order_date as date_of_order,deliveryShift');
...
//	$fsorders = $this->db->query("select order_id,user_id,shipping_address,delivery_status,id,order_type,assigned_to,delivery_date as deliverydate,order_date as date_of_order,deliveryShift from tbl_free_sample_orders where order_status='Success' order by id desc")->result();
	...
	$this->db->select('order_id,user_id,shipping_address,delivery_status,id,order_type,assigned_to,delivery_date as deliverydate,order_date as date_of_order,deliveryShift');
	$this->db->from('tbl_free_sample_orders');
	$this->db->where("order_status","Success");
...
public function get_success_orders($pid,$cat){
	...
	$orders = $this->db->query("select * from orders where payment_status='Success'")->result();
...
//		echo $o->deliveryonce_date;
			...
			if($o->order_type == "deliveryonce"){
				...
				$sdata = $this->db->get_where("tbl_subscribed_deliveries",array("order_id"=>$o->order_id,"pause_status"=>"Inactive","deliver_status !="=>"Cancelled"))->result();
